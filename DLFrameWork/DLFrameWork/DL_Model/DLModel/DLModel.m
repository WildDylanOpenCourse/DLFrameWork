...
#error The DLModel framework is ARC only, you can enable ARC on per file basis.
#endif
...
static Class DLModelClass = NULL;

#pragma mark - model cache
static DLKeyMapper* globalKeyMapper = nil;

#pragma mark - DLModel implementation
...
+(void)load
{
    ...
    dispatch_once(&once, ^{
		...
		@autoreleasepool {
            ...
            DLModelClass = NSClassFromString(NSStringFromClass(self));
		}
    });
}
...
-(instancetype)initWithData:(NSData *)data error:(NSError *__autoreleasing *)err
{
    //check for nil input
    if (!data) {
        if (err) *err = [DLModelError errorInputIsNil];
        return nil;
    }
    //read the json
    ...
    if (initError) {
        if (err) *err = [DLModelError errorBadJSON];
        return nil;
    }
...
-(id)initWithString:(NSString*)string error:(DLModelError**)err
{
    DLModelError* initError = nil;
...
-(id)initWithString:(NSString *)string usingEncoding:(NSStringEncoding)encoding error:(DLModelError**)err
{
    //check for nil input
    if (!string) {
        if (err) *err = [DLModelError errorInputIsNil];
...
-(id)initWithDictionary:(NSDictionary*)dict error:(NSError**)err
{
    ...
    if (![dict isKindOfClass:[NSDictionary class]]) {
        if (err) *err = [DLModelError errorInvalidDataWithMessage:@"Attempt to initialize DLModel object using initWithDictionary:error: but the dictionary parameter was not an 'NSDictionary'."];
        return nil;
    }
    ...
    if (!self) {
        ...
        if (err) *err = [DLModelError errorModelIsInvalid];
        return nil;
    }
...
-(DLKeyMapper*)__keyMapper
{
    //get the model key mapper
    return objc_getAssociatedObject(self.class, &kMapperObjectKey);
}

-(BOOL)__doesDictionary:(NSDictionary*)dict matchModelWithKeyMapper:(DLKeyMapper*)keyMapper error:(NSError**)err
{
    ...
    if (keyMapper || globalKeyMapper) {
        ...
        for (DLModelClassProperty* property in [self __properties__]) {
    ...
    if (![requiredProperties isSubsetOfSet:incomingKeys]) {
        ...
        if (err) *err = [DLModelError errorInvalidDataWithMissingKeys:requiredProperties];
        return NO;
    }
...
-(NSString*)__mapString:(NSString*)string withKeyMapper:(DLKeyMapper*)keyMapper importing:(BOOL)importing
{
    if (keyMapper) {
        //custom mapper
...
-(BOOL)__importDictionary:(NSDictionary*)dict withKeyMapper:(DLKeyMapper*)keyMapper validation:(BOOL)validation error:(NSError**)err
{
    ...
    for (DLModelClassProperty* property in [self __properties__]) {
        ...
        if (isNull(jsonValue)) {
            ...
            if (err) {
                ...
                DLModelError* dataErr = [DLModelError errorInvalidDataWithMessage:msg];
        ...
        if (property) {
            ...
            // 1) check if property is itself a DLModel
            if ([self __isDLModelSubClass:property.type]) {
                
                //initialize the property's model, store it
                DLModelError* initErr = nil;
...
-(BOOL)__isDLModelSubClass:(Class)class
{
#ifdef UNIT_TESTING
    return [@"DLModel" isEqualToString: NSStringFromClass([class superclass])];
#else
    return [class isSubclassOfClass:DLModelClass];
#endif
}
...
-(NSMutableSet*)__requiredPropertyNames
{
    ...
    if (!classRequiredPropertyNames) {
        ...
        [[self __properties__] enumerateObjectsUsingBlock:^(DLModelClassProperty* p, NSUInteger idx, BOOL *stop) {
...
-(void)__inspectProperties
{
    ...
    // inspect inherited properties up to the DLModel class
    while (class != [DLModel class]) {
        ...
        for (unsigned int i = 0; i < propertyCount; i++) {
            ...
            DLModelClassProperty* p = [[DLModelClassProperty alloc] init];
            ...
            //the property must be a primitive
                ...
                if (![allowedPrimitiveTypes containsObject:propertyType]) {
                    ...
                    @throw [NSException exceptionWithName:@"DLModelProperty type not allowed"
                                                   reason:[NSString stringWithFormat:@"Property type of %@.%@ is not supported by DLModel.", self.class, p.name]
                                                 userInfo:nil];
                }
            ...
            //few cases where DLModel will ignore properties automatically
        ...
        //(will do that until it reaches the root class - DLModel)
...
-(id)__transform:(id)value forProperty:(DLModelClassProperty*)property error:(NSError**)err
{
    Class protocolClass = NSClassFromString(property.protocol);
    if (!protocolClass) {

        //no other protocols on arrays and dictionaries
        //except DLModel classes
        if ([value isKindOfClass:[NSArray class]]) {
            @throw [NSException exceptionWithName:@"Bad property protocol declaration"
                                           reason:[NSString stringWithFormat:@"<%@> is not allowed DLModel property protocol, and not a DLModel class.", property.protocol]
                                         userInfo:nil];
        }
        return value;
    }
    
    //if the protocol is actually a DLModel class
    if ([self __isDLModelSubClass:protocolClass]) {
        ...
        if ([property.type isSubclassOfClass:[NSArray class]]) {
			...
			if(![[value class] isSubclassOfClass:[NSArray class]])
			{
				if(err != nil)
				{
					...
					DLModelError* typeErr = [DLModelError errorInvalidDataWithTypeMismatch:mismatch];
            ...
            if (property.convertsOnDemand) {
                ...
                value = [[DLModelArray alloc] initWithArray:value modelClass:[protocolClass class]];
                
            } else {
                //one shot conversion
				DLModelError* arrayErr = nil;
...
-(id)__reverseTransform:(id)value forProperty:(DLModelClassProperty*)property
{
    Class protocolClass = NSClassFromString(property.protocol);
    ...
    if ([self __isDLModelSubClass:protocolClass]) {
        ...
        if (property.type == [NSArray class] || property.type == [NSMutableArray class]) {
            ...
            for (NSObject<AbstractDLModelProtocol>* model in (NSArray*)value) {
        ...
        if (property.type == [NSDictionary class] || property.type == [NSMutableDictionary class]) {
            ...
            for (NSString* key in [(NSDictionary*)value allKeys]) {
                id<AbstractDLModelProtocol> model = value[key];
...
-(BOOL)__customSetValue:(id<NSObject>)value forProperty:(DLModelClassProperty*)property
{
...
-(BOOL)__customGetValue:(id<NSObject>*)value forProperty:(DLModelClassProperty*)property
{
...
-(NSDictionary*)toDictionaryWithKeys:(NSArray*)propertyNames
{
    ...
    for (DLModelClassProperty* p in properties) {
        ...
        if ([value isKindOfClass:DLModelClass]) {

            //recurse models
            value = [(DLModel*)value toDictionary];
            [tempDictionary setValue:value forKeyPath: keyPath];
...
-(NSData*)toJSONDataWithKeys:(NSArray*)propertyNames
{
	...
	@catch (NSException *exception) {
		//this should not happen in properly design DLModel
...
+(NSMutableArray*)arrayOfDictionariesFromModels:(NSArray*)array
{
    ...
    for (id<AbstractDLModelProtocol> object in array) {
...
-(NSString*)description
{
    ...
    for (DLModelClassProperty *p in [self __properties__]) {
...
+(void)setGlobalKeyMapper:(DLKeyMapper*)globalKeyMapperParam
{
